...
forcing_data = '/data/ssd2/streamflow-ml-data-operational/operational-forcing-data/historical'
...
data_loader_test = DataLoader(testStreamflowData, batch_size=1, shuffle=False, num_workers=64, collate_fn=collate_fn)
...
for k in range(1, num_models + 1):
    ...
    for i in range(len(data_loader_test)):
        with torch.no_grad():  # Disable gradient computation
            ...
            output_file_path = os.path.join(forcing_data, f'inference-results-fold-{k}', f'inference-results-' + path[0]).replace('.parquet', '.csv')
