...
python raster_inference.py \
  ...
  --model_py_dir /home/zhoylman/soil-moisture-ml/py \
  --model_class uNet \
...
python raster_inference.py \
  ...
  --model_py_dir /home/zhoylman/soil-moisture-ml/py --model_class uNet \
  --device cuda
"""
...
def run_inference_on_raster(
    ...
    *,
    ...
    model_kind: str = "unet",                 # "unet" | "seq"
    seq_chunk_px: int = 262144               # pixels per chunk for seq adapter
) -> str:
    """
    Run inference over a single multi-band raster and write prediction GeoTIFF.

    Output:
      - if write_mode='multiband': one GeoTIFF with count=T_out bands (e.g., 180)
      - if write_mode='single': single-band GeoTIFF (first timestep)
    """
    ...
    with rasterio.open(inp_path) as src:
        ...
        for i in tqdm(range(0, len(tiles), batch_size), desc=f"Infer {Path(inp_path).name}"):
            ...
            for (x, y, w, h) in batch_tiles:
                ...
                data = src.read(window=win, out_dtype="float32")  # [C,H,W]
                ...
                # shape -> tensor
                if prep.ndim == 4:      # [T,C,H,W]
                    t = torch.from_numpy(prep).unsqueeze(0).to(device, non_blocking=True)  # [B,T,C,H,W]
                elif prep.ndim == 3:    # [C',H,W] (flat or no time)
                    t = torch.from_numpy(prep).unsqueeze(0).to(device, non_blocking=True)  # [B,C',H,W]
                else:
            ...
            # Pad tensors in batch to same H,W (edge tiles)
            max_h = max(h for h, _ in shapes)
            max_w = max(w for _, w in shapes)
            t0 = batch_inputs[0]

            if t0.ndim == 5:  # [B,T,C,H,W]
                B, T_in, C_in, _, _ = t0.shape
                batch_tensor = torch.zeros((len(batch_inputs), T_in, C_in, max_h, max_w),
                                           dtype=torch.float32, device=device)
                for bi, (t, (h, w)) in enumerate(zip(batch_inputs, shapes)):
                    batch_tensor[bi, :, :, :h, :w] = t
            else:             # [B,C',H,W]
                B, C_in, _, _ = t0.shape
                batch_tensor = torch.zeros((len(batch_inputs), C_in, max_h, max_w),
                                           dtype=torch.float32, device=device)
                for bi, (t, (h, w)) in enumerate(zip(batch_inputs, shapes)):
                    batch_tensor[bi, :, :h, :w] = t
            ...
            with torch.no_grad():
                if model_kind == "seq":
                    if batch_tensor.dim() == 4:
                        raise RuntimeError("Sequence model requires [B,T,C,H,W] input; got flattened 4D.")
                    pred_t = seq_model_pixelwise_infer(
                        model, batch_tensor, device=device, chunk_px=seq_chunk_px
                    )  # [B,T_out,H,W] CPU tensor
                else:
                    pred = model(batch_tensor)
                    # Normalize to [B,T_out,H,W]
                    if pred.dim() == 5:             # [B,T,1,H,W] or [B,T,C,H,W]
                        if pred.shape[2] == 1:
                            pred = pred[:, :, 0, :, :]
                        else:
                            pred = pred[:, :, 0, :, :]
                    elif pred.dim() == 4:           # [B,T,H,W] or [B,1,H,W]
            ...
            pred_np = pred_t.numpy()  # [B, T_out, H, W], B==len(batch_tiles)
            if T_out is None:
            ...
            for bi, ((x, y), (h, w)) in enumerate(zip(offsets, shapes)):
                tile_pred_all = pred_np[bi, :, :h, :w]  # [T_out,h,w]
                m = nan_masks[bi]
                if m.shape != (h, w):
                    m = m[:h, :w]
                m3 = np.broadcast_to(m, (tile_pred_all.shape[0], h, w))
