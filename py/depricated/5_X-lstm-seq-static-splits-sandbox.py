...
def train_and_evaluate(site_number):
    ...
    model_name = f'LSTM_{site_number}_sites_{datetime.datetime.now().strftime("%m_%d_%Y-%H_%M_%S_%f")}'
    ...
    template = StreamflowTrainDatasetTemplate(
        csv_file=get_first_file_path('/data/ssd2/soil-moisture-ml/full-dataloader')
    )

    input_size = template.x_tensor.size()[2]

    soilMoistureData = StreamflowTrainDatasetXY(
        path_csv=f'/data/ssd2/soil-moisture-ml/split-definitions/train_{site_number}.csv',
        dim_1=template.x_tensor.shape[1],
        dim_2=template.x_tensor.shape[2]
    )

    data_loader = DataLoader(soilMoistureData, batch_size=batch_size, shuffle=True, num_workers=64)

    streamflowTestData = StreamflowTrainDatasetXY_Test(
        path_csv='/data/ssd2/soil-moisture-ml/split-definitions/validation_100.csv',
        dim_2=template.x_tensor.shape[2]
    )

    data_loader_test = DataLoader(streamflowTestData, batch_size=1, shuffle=True, num_workers=1)

    #BiLSTM_model = BiLSTM(num_classes, input_size, hidden_size, dropout, num_layers, linear_nodes, template.x_tensor.shape[2]).to('cuda')
        ...
        if epoch % 10 == 0:
            ...
            for X_batch, y_batch, path in data_loader_test:
                with torch.no_grad():
                    ...
                    test_loss = kge_loss(outputs_test, y_batch.to('cuda'))
                    test_losses.append(test_loss.item())

                    yhat = outputs_test.cpu().view(-1).numpy()
                    yobs = y_batch.cpu().view(-1).numpy()
            ...
            # Horizontal reference lines every 0.1 from 0 to 1
            for y in np.arange(0.1, 1.0, 0.1):
                plt.axhline(y=y, linestyle='--', color='gray', linewidth=0.5)

            # Plot the KGE percentiles
            plt.plot(test_kge_df['epoch'], test_kge_df['median_kge'], label='Median KGE', color='green', linewidth=2)
            plt.plot(test_kge_df['epoch'], test_kge_df['lower_kge'], label='25th Percentile', color='red',
                     linewidth=1.5)
            plt.plot(test_kge_df['epoch'], test_kge_df['upper_kge'], label='75th Percentile', color='blue',
                     linewidth=1.5)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{out_dir}/KGE_curve.png')
    plt.clf()

    test_losses_df.to_csv(f'{out_dir}/test_losses.csv', index=False)
    ...
    for i, (X, y, path) in enumerate(data_loader_test):
        with torch.no_grad():
            ...
            yhat = BiLSTM_model(X.to('cuda')).cpu().detach().numpy()[0, :, 0]
            yobs = y.numpy()[0, :]
    ...
    kge_out = np.array(kge_out, dtype=np.float64)  # Ensure 1D float array
    ...
    plt.scatter(median_kge, median_cdf_value, color='red', zorder=5)
    plt.xlabel("KGE")
    plt.ylabel("CDF")
...
site_numbers = ['100', '200', '300', '400', '500', '600', '700', '740']
#site_numbers = ['740']
