...
make_figure = function(d) {
  ...
  depth_name  = c("Shallow Soil Moisture (0-10 cm)", "Mid-depth Soil Moisture (10-50 cm)")[d]
  ...
  plot = ggplot(curve, aes(x = n_sites, y = median)) +
    ...
    labs(title = depth_name, x = "Number of Sites", y = "Median KGE") +
    theme_bw(base_size = 15) +
    theme(plot.title = element_text(face = "bold", hjust = 0.5),
          axis.title = element_text(face = "bold"), axis.text = element_text(color = "black"))
  message(glue("[{depth_2}] SPoRT-LIS median KGE = {round(sport_kge,3)} | {nrow(curve)} points"))
  plot
}

plan(multisession, workers = 20)
plots = lapply(1:2, make_figure)
plan(sequential)

library(patchwork)
combined = (plots[[1]] | plots[[2]]) +
  patchwork::plot_annotation(title = "Out-of-sample KGE vs. number of training sites", tag_levels = "a",
                             theme = theme(plot.title = element_text(face = "bold", hjust = 0.5, size = 17)))
ggsave(glue("{figs_dir}/kge_vs_training_sites_combined.png"), combined, width = 13, height = 5.5, dpi = 300, bg = "white")
message("Wrote kge_vs_training_sites_combined.png")
