...
exclude_networks = c("OK Mesonet", "NEON")   # not used (NEON = bad obs quality)
...
# Main analysis meta has the FULL UMRB Mesonet (267) + SNTL/SCAN/USCRN. NEON is
# excluded (bad obs quality), along with OK Mesonet.
obs_meta = read_csv(file.path(data_root, "observations/final-soil-moisture-data-generalized-meta.csv"),
...
p_pretrain = ggplot() +
  geom_sf(data = us_sf, fill = "transparent", color = "black", linewidth = 0.4) +
...
# ---- combined two-panel: side by side (works better on the page) ----
if (requireNamespace("patchwork", quietly = TRUE)) {
  library(patchwork)
  combined = (p_pretrain | p_obs) + patchwork::plot_annotation(tag_levels = "a")
  ggsave(file.path(figs_dir, "site_map_combined.png"), combined, width = 15, height = 6, dpi = 300, bg = "white")
}
