...
seq_covariates_raw = readr::read_csv("/data/ssd2/soil-moisture-ml/seq-data/pretraining-sites-seq-data.csv")

sites_with_data = colnames(seq_covariates_raw) %>% unique
 
roi = read_sf("/data/ssd2/soil-moisture-ml/random-pretraining-roi/pretraining-roi.geojson") %>%
  mutate(latitude = st_coordinates(geometry)[,2],
         longitude = st_coordinates(geometry)[,1]) %>%
  filter(site_id %in% sites_with_data)
...
readr::write_csv(min_max_seq, '/data/ssd2/soil-moisture-ml/min-max-definitions/seq-min-max-definitions-pretrain-shallow.csv')

#------------------------------------------------------------
# Load target variable (soil moisture)
#------------------------------------------------------------

target = readr::read_csv("/data/ssd2/soil-moisture-ml/observations/pretraining-sites-raw-sport-depth-0-10cm.csv") 

#------------------------------------------------------------
# Import and normalize static covariates
#------------------------------------------------------------

static_covariates = readr::read_csv("/data/ssd2/soil-moisture-ml/static-data/pretrain-sites-static-data.csv") %>%
  tidyr::drop_na() %>%
  dplyr::mutate_all(~ replace(., is.infinite(.), NA)) %>%
  dplyr::left_join(roi %>% st_drop_geometry(), by = 'site_id') %>%
  drop_na() %>%
...
min_max_static = readr::read_csv("/data/ssd2/soil-moisture-ml/static-data/pretrain-sites-static-data.csv") %>%
  tidyr::drop_na() %>%
  dplyr::left_join(roi %>% st_drop_geometry(), by = 'site_id') %>%
  dplyr::summarise(across(everything(), min, na.rm = TRUE)) %>%
  dplyr::bind_rows(
    readr::read_csv("/data/ssd2/soil-moisture-ml/static-data/pretrain-sites-static-data.csv") %>%
      tidyr::drop_na() %>%
      dplyr::left_join(roi %>% st_drop_geometry(), by = 'site_id') %>%
      dplyr::summarise(across(everything(), max, na.rm = TRUE))
  ) %>%
...
readr::write_csv(min_max_static, '/data/ssd2/soil-moisture-ml/min-max-definitions/static-min-max-definitions-pretrain-shallow.csv')
#------------------------------------------------------------
...
temp_out = foreach(b = unique(out$site_id),
                   ...
                   .options.snow = opts) %dopar% {
                     ...
                     temp_target = target %>%
                       dplyr::select(var, time, b)  %>%
                       #mutate(time = as.Date(time),
                        #      depth = ifelse(var == 'SPoRT_raw_0-10cm', .05,
                        #                     ifelse(var == 'SPoRT_raw_10-40cm', .25, .70))) %>%
                       dplyr::select(time, soil_moisture = b)#, depth)
                       ...
                       for (i in seq_along(train_temp)) {
                         if (nrow(train_temp[[i]]) == group_size && # if length is group_size
                           ...
                           write_csv(train_temp[[i]] %>% dplyr::select(-group_id),
                                     paste0('/data/ssd2/soil-moisture-ml/full-dataloader-pretrain-shallow/basin-group-',
                                            b, '_', i,'-train.csv'))
                         }
                       }
                     
                     # for(d in c(0.05, .25, .70)){
                     #   print(d)
                     #   train_temp = temp_out_data %>%
                     #     left_join(
                     #       temp_target %>%
                     #         filter(depth == d)
                     #     ) %>%
                     #     dplyr::select(site_id, time, soil_moisture, depth, everything()) %>%
                     #     dplyr::select(-c(site_id)) %>%
                     #     mutate(circular_yday = convert_julian_to_circular(lubridate::yday(time))) %>%
                     #     drop_na() %>%
                     #     assign_group_id(stride_length = stride_length, group_size = group_size)
                     ...
                     #         all(diff(train_temp[[i]]$time) == 1)) { # and if time is complete and sequential
                     #       write_csv(train_temp[[i]] %>% dplyr::select(-group_id),
                     #                 paste0('/data/ssd2/soil-moisture-ml/full-dataloader-pretrain/basin-group-',
                     #                        b, '_', i, '_', d*100, '-train.csv'))
                     #     }
                     #   }
...
all_files = list.files("/data/ssd2/soil-moisture-ml/full-dataloader-pretrain-shallow", full.names = T) %>%
  as_tibble() %>%
  mutate(site_id = str_extract(value, "(?<=basin-group-)[^_]+_[^_]+")) %>%
  rename(path = value)
...
generate_site_splits = function(all_sites,
                                all_files,
                                validation_n = 1000,
                                out_dir = '/data/ssd2/soil-moisture-ml/split-definitions-pretrain-shallow',
                                seed = 69) {
  ...
  # Save training set (all remaining sites)
  train_sites = setdiff(all_sites, validation_sites)
  
  readr::write_csv(
    all_files %>% dplyr::filter(site_id %in% train_sites),
    file = file.path(out_dir, paste0("train_", length(train_sites), ".csv"))
  )
  
  message("Training and validation splits written to: ", out_dir)
}
