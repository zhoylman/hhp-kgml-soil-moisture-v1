...
# Function to apply pre-defined min-max normalization
min_max_normalize_pre_established = function(df, min_max_data) {
  min_max_data = dplyr::as_tibble(min_max_data)
  normalized_cols = list()
  for (col in colnames(df)) {
    if (is.Date(df[[col]]) || is.character(df[[col]])) {
      normalized_cols[[col]] = df[[col]]
    } else {
      min_val = dplyr::filter(min_max_data, name == col & min_max_id == "min") %>% dplyr::pull(value)
      max_val = dplyr::filter(min_max_data, name == col & min_max_id == "max") %>% dplyr::pull(value)
      normalized_cols[[col]] = (df[[col]] - min_val) / (max_val - min_val)
    }
  }
  return(dplyr::as_tibble(normalized_cols))
}
...
seq_covariates_raw = readr::read_csv("/data/ssd2/soil-moisture-ml/seq-data/observational-sites-seq-data.csv")
                            ...
                            .options.snow = opts) %dopar% {
                              ...
                              seq_covariates_out = seq_covariates_raw %>%
                                ...
                                dplyr::mutate(
                                  ...
                                  precip_roll_sum_mid         = zoo::rollsum(pr, align = 'right', k = 60, fill = NA),
                                  pet_roll_sum_mid            = zoo::rollsum(pet, align = 'right', k = 60, fill = NA),
                                  precip_roll_sum_long        = zoo::rollsum(pr, align = 'right', k = 365, fill = NA),
                                  pet_roll_sum_long           = zoo::rollsum(pet, align = 'right', k = 365, fill = NA),
                                  precip_roll_sum_long_long   = zoo::rollsum(pr, align = 'right', k = 730, fill = NA),
                                  pet_roll_sum_long_long      = zoo::rollsum(pet, align = 'right', k = 730, fill = NA),
                                  ...
                                  def_long_long               = precip_roll_sum_long_long - pet_roll_sum_long_long
                                ) %>%
                                dplyr::ungroup() %>%
                                dplyr::arrange(site_id, year)
...
site_loc_full = readr::read_csv("/data/ssd2/soil-moisture-ml/observations/final-soil-moisture-data-generalized-meta.csv")  

#------------------------------------------------------------
# Load and transform target variable (streamflow)
#------------------------------------------------------------

# target = read_csv("/data/ssd2/soil-moisture-ml/observations/final-soil-moisture-data-raw-depths.csv") %>% 
#   filter(depth == 0.2) %>%
#   #dplyr::select(site_id, time = date, depth, soil_moisture) %>%
#   dplyr::select(site_id, time = date, soil_moisture) %>%
#   filter(site_id %in% site_loc$site_id) %>%
#   drop_na()

target = read_csv("/data/ssd2/soil-moisture-ml/observations/final-soil-moisture-data-generalized.csv")%>% 
  filter(generalized_depth == 'Middle') %>%
  #dplyr::select(site_id, time = date, depth, soil_moisture) %>%
  dplyr::select(site_id, time = date, soil_moisture) %>%
  filter(site_id %in% site_loc$site_id) %>%
  drop_na()
# 
# sport_raw = read_csv("/data/ssd2/soil-moisture-ml/observations/observational-sites-raw-sport-depth-10-40cm.csv") |> 
#   pivot_longer(cols = -c(var, time)) |>
#   rename(site_id = name, sport = value) |>
#   mutate(time = as.Date(time)) |>
#   select(site_id,time,sport)
# 
# joined = left_join(target, sport_raw) |>
#   drop_na()
# 
# val_sites = read_csv("/data/ssd2/soil-moisture-ml/split-definitions/validation_100.csv") %$%
#   site_id |>
#   unique()
# 
# kge_per_site = joined |>
#   group_by(site_id) |>
#   summarise(
#     KGE = hydroGOF::KGE(sim = sport, obs = soil_moisture, na.rm = TRUE),
#     .groups = "drop"
#   ) |>
...
static_covariates = readr::read_csv("/data/ssd2/soil-moisture-ml/static-data/all-sites-static-data.csv") %>%
  ...
  dplyr::left_join(site_loc, by = 'site_id') %>%
  min_max_normalize_pre_established(min_max_static_pretrain)
...
out = seq_covariates %>%
  dplyr::mutate_all(~ replace(., is.infinite(.), NA)) %>%
  tidyr::drop_na() %>%
  min_max_normalize_pre_established(min_max_seq_pretrain) %>%
  dplyr::as_tibble() %>%
  ...
  dplyr::select(site_id, time, everything()) %>%
  tidyr::drop_na()
...
# Write out full sequence datasets per site and available depth
                     ...
                     for (i in seq_along(train_temp)) {
                       if (nrow(train_temp[[i]]) == group_size && # if length is group_size
                         ...
                         write_csv(train_temp[[i]] %>% dplyr::select(-group_id),
                                   paste0('/data/ssd2/soil-moisture-ml/full-dataloader-middle/basin-group-',
                                          b, '_', i,'-train.csv'))
                       }
                     }
                     
                     rm(train_temp, temp_out_data, temp_target); gc()
                   }
...
all_files = list.files("/data/ssd2/soil-moisture-ml/full-dataloader-middle", full.names = T) %>%
  as_tibble() %>%
...
#                                 validation_n = 40,
#                                 train_step = 50,
#                                 out_dir = '/data/ssd2/soil-moisture-ml/split-definitions-middle',
#                                 seed = 69) {
#   
...
generate_site_splits = function(all_sites,
  ...
  # how many unique sites are available
  n_available = length(all_sites)
  
  # build train sizes (step-based sequence + full pool)
  train_sizes = seq(train_step, n_available - 1, by = train_step)
  train_sizes = unique(c(train_sizes, n_available))
  
  # for each train size: sample training sites; validation = all remaining sites
  for (n_train in train_sizes) {
    train_sites = sample(all_sites, n_train)
    val_sites   = setdiff(all_sites, train_sites)   # <- all remaining as validation
    
    # write train
    readr::write_csv(
      dplyr::filter(all_files, site_id %in% train_sites),
      file = file.path(out_dir, glue::glue("train_{n_train}.csv"))
    )
    
    # write validation paired with this train size
    readr::write_csv(
      dplyr::filter(all_files, site_id %in% val_sites),
      file = file.path(out_dir, glue::glue("validation_for_train_{n_train}.csv"))
    )
  }
  
  message("Site splits (train + exhaustive validation) saved to: ", out_dir)
}
...
generate_kfold_site_splits = function(all_sites,
                                      all_files,
                                      k = 10,
                                      out_dir = '/data/ssd2/soil-moisture-ml/split-definitions-kfold-middle',
                                      seed = 69) {
  ...
  shuffled_sites = sample(all_sites)
  folds = cut(seq_along(shuffled_sites), breaks = k, labels = FALSE)
  
  for (fold in 1:k) {
    validation_sites = shuffled_sites[folds == fold]
    training_sites = setdiff(shuffled_sites, validation_sites)
    
    readr::write_csv(
      all_files %>% dplyr::filter(site_id %in% validation_sites),
      file = file.path(out_dir, glue::glue("validation_split_fold_{fold}.csv"))
    )
    
    readr::write_csv(
      all_files %>% dplyr::filter(site_id %in% training_sites),
      file = file.path(out_dir, glue::glue("train_split_fold_{fold}.csv"))
    )
    
    message(glue::glue("Fold {fold}: {length(training_sites)} train / {length(validation_sites)} validate"))
  }
}

generate_kfold_site_splits(
  all_sites = unique(all_files$site_id),
  all_files = all_files,
  k = 10,
  out_dir = '/data/ssd2/soil-moisture-ml/split-definitions-kfold-middle'
